...
type MessagePayload = { id: number; senderId: number; recipientId: number; body?: string; deliveredAt: string | null; media?: { url: string; kind: "image" | "video"; name: string } | null; mediaItems?: Array<{ url: string; caption?: string | null }>; callEvent?: { callId: string; status: string } | null };
...
describe.skipIf(!hasCredentials)("realtime chat feedback", () => {
  it("forwards typing and read receipts between two authenticated users", async () => {
    ...
    const captionIncoming = once<MessagePayload>(secondSocket, "chat:new", (payload) => Boolean(payload.mediaItems?.some((item) => item.caption === "Chú thích riêng cho ảnh")));
    const captionReply = await new Promise<{ ok: boolean; message?: MessagePayload }>((resolve) => firstSocket.emit("chat:send", { recipientId: second.user.id, body: "Chú thích chung", mediaItems: [{ ...media, caption: "Chú thích riêng cho ảnh" }] }, resolve));
    expect(captionReply.ok).toBe(true);
    await expect(captionIncoming).resolves.toMatchObject({ body: "Chú thích chung", mediaItems: [{ url: media.url, caption: "Chú thích riêng cho ảnh" }] });
    ...
    const quickCallId = `call-quick-reply-${Date.now()}`;
    const quickOffer = once<{ callId: string }>(secondSocket, "call:offer", (payload) => payload.callId === quickCallId);
    firstSocket.emit("call:offer", { toUserId: second.user.id, callId: quickCallId, withVideo: false });
    await expect(quickOffer).resolves.toMatchObject({ callId: quickCallId });
    const quickReplyIncoming = once<MessagePayload>(firstSocket, "chat:new", (payload) => payload.senderId === second.user.id && payload.body === "Tôi sẽ gọi lại trong ít phút.");
    secondSocket.emit("call:hangup", { toUserId: first.user.id, callId: quickCallId, reason: "declined", quickReply: "Tôi sẽ gọi lại trong ít phút." });
    await expect(quickReplyIncoming).resolves.toMatchObject({ body: "Tôi sẽ gọi lại trong ít phút." });
    const callHistoryResponse = await fetch(`${apiUrl!.replace(/\/$/, "")}/api/calls?peerId=${first.user.id}`, { headers: { Authorization: `Bearer ${second.token}` } });
    expect(callHistoryResponse.ok).toBe(true);
    expect((await callHistoryResponse.json() as Array<{ id: string }>).some((item) => item.id === callId)).toBe(true);
