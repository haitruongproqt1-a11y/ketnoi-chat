...
import { ActivityIndicator, Animated, FlatList, Image, KeyboardAvoidingView, Modal, PanResponder, Platform, Pressable, StyleSheet, Text, TextInput, View } from "react-native";
...
export default function ChatScreen() {
  ...
  const { onlineIds, lastMessage, lastTyping, lastReadReceipt, lastDeliveryReceipt, lastMediaRecall, lastMessageRecall, lastDeletedMessage, lastClearedConversation, sendMessage, sendTyping } = useMobileSocket();
  ...
  const [forwarding, setForwarding] = useState<MobileMessage | null>(null);
  const [forwardTargets, setForwardTargets] = useState<MobileUser[]>([]);
  ...
  const [fullScreenImage, setFullScreenImage] = useState<NonNullable<MobileMessage["media"]> | null>(null);
  ...
  const [actionMessage, setActionMessage] = useState<MobileMessage | null>(null);
  ...
  useEffect(() => {
    if (!lastMessageRecall) return;
    const belongs = (lastMessageRecall.senderId === peerId && lastMessageRecall.recipientId === user?.id) || (lastMessageRecall.recipientId === peerId && lastMessageRecall.senderId === user?.id);
    if (belongs) setMessages((current) => current.map((message) => message.id === lastMessageRecall.id ? lastMessageRecall : message));
  }, [lastMessageRecall, peerId, user?.id]);
  useEffect(() => {
    if (!lastDeletedMessage || lastDeletedMessage.peerId !== peerId) return;
    setMessages((current) => current.filter((message) => message.id !== lastDeletedMessage.messageId));
  }, [lastDeletedMessage, peerId]);
  useEffect(() => {
    if (!lastClearedConversation || lastClearedConversation.peerId !== peerId) return;
    setMessages([]);
  }, [lastClearedConversation, peerId]);
  useEffect(() => () => {
    if (typingTimeout.current) clearTimeout(typingTimeout.current);
  ...
  const send = async () => {
    ...
    if (!body || !peer) return;
    const quotedBody = replyingTo?.body?.trim() || (replyingTo?.media ? "Tệp đính kèm" : "Tin nhắn");
    const payload = replyingTo ? `↩ ${quotedBody.slice(0, 140)}\n${body}` : body;
    setDraft("");
    if (typingTimeout.current) clearTimeout(typingTimeout.current);
    sendTyping(peer.id, false);
    try {
      const message = await sendMessage(peer.id, payload);
  ...
  const pickMedia = async () => {
    ...
    try {
      ...
      setPendingMedia(result.assets.slice(0, 8).map((asset) => ({ asset, caption: "" })));
      setError("");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Không thể mở thư viện ảnh hoặc video."); }
  };
  const pickFile = async () => {
    if (!peer || !token || uploading) return;
    try {
      const result = await DocumentPicker.getDocumentAsync({ type: ["application/pdf", "text/*", "application/zip", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.openxmlformats-officedocument.presentationml.presentation"], copyToCacheDirectory: true, multiple: false });
      if (result.canceled || !result.assets[0]) return;
      if ((result.assets[0].size ?? 0) > 12 * 1024 * 1024) throw new Error("Tệp đính kèm cần nhỏ hơn 12 MB.");
      setPendingFile(result.assets[0]); setError("");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Không thể chọn tệp đính kèm."); }
  };
  const updatePendingCaption = (uri: string, caption: string) => setPendingMedia((items) => items.map((item) => item.asset.uri === uri ? { ...item, caption } : item));
  const sendPendingMedia = async () => {
    if (!peer || !token || uploading || !pendingMedia.length) return;
    try {
      ...
      for (const [index, item] of pendingMedia.entries()) {
        ...
        const start = index / pendingMedia.length;
        const optimized = await optimizeMedia({ uri: asset.uri, name: asset.fileName ?? `${kind}-${Date.now()}-${index}.${kind === "video" ? "mp4" : "jpg"}`, mimeType, size, kind, width: asset.width, height: asset.height }, (progress) => setUploadProgress(start + progress * 0.55 / pendingMedia.length), (cancel) => { uploadCancellation.current = cancel; });
        ...
        const media = await mobileApi.uploadMedia(token, optimized, (progress) => setUploadProgress(start + 0.55 / pendingMedia.length + progress * 0.45 / pendingMedia.length), (cancel) => { uploadCancellation.current = cancel; });
        uploaded.push({ ...media, thumbnailUrl, caption: item.caption.trim() || null });
      }
  ...
  const sendPendingFile = async () => {
    if (!peer || !token || uploading || !pendingFile) return;
    try {
      setUploading(true); setUploadProgress(0.02); setError(""); uploadWasCanceled.current = false;
      const uploaded = await mobileApi.uploadMedia(token, { uri: pendingFile.uri, name: pendingFile.name, mimeType: pendingFile.mimeType ?? "application/octet-stream", size: pendingFile.size ?? 1 }, setUploadProgress, (cancel) => { uploadCancellation.current = cancel; });
      const message = await sendMessage(peer.id, draft.trim(), uploaded);
      setDraft(""); setPendingFile(null); void AsyncStorage.removeItem(draftStorageKey);
      setMessages((current) => current.some((item) => item.id === message.id) ? current : [...current, message]);
    } catch (reason) { setError(uploadWasCanceled.current ? "Đã hủy tải tệp." : reason instanceof Error ? reason.message : "Không thể gửi tệp đính kèm."); }
    finally { setUploading(false); setUploadProgress(0); uploadCancellation.current = null; uploadWasCanceled.current = false; }
  };
  ...
  const deleteMessage = async (message: MobileMessage) => { if (!token) return; try { await mobileApi.deleteMessage(token, message.id); setMessages((current) => current.filter((item) => item.id !== message.id)); setActionMessage(null); } catch { setError("Không thể xóa tin nhắn. Vui lòng thử lại."); } };
  const recallMessage = async (message: MobileMessage) => { if (!token || message.senderId !== user?.id) return; try { const recalled = await mobileApi.recallMessage(token, message.id); setMessages((current) => current.map((item) => item.id === recalled.id ? recalled : item)); setActionMessage(null); } catch (reason) { setError(reason instanceof Error ? reason.message : "Không thể thu hồi tin nhắn."); } };
  const clearConversation = async () => { if (!token || !peer) return; try { await mobileApi.clearConversation(token, peer.id); setMessages([]); setShowClearConfirm(false); } catch { setError("Không thể làm mới hội thoại. Vui lòng thử lại."); } };
  const beginForward = async (message: MobileMessage) => { if (!token || (!message.media && !message.mediaItems?.length)) return; try { setForwardTargets((await mobileApi.friends(token)).filter((friend) => friend.id !== peerId)); setForwardTargetIds([]); setForwarding(message); } catch { setError("Không tải được danh sách để chuyển tiếp."); } };
  const toggleForwardTarget = (targetId: number) => setForwardTargetIds((current) => current.includes(targetId) ? current.filter((id) => id !== targetId) : [...current, targetId]);
  const forwardToSelected = async () => { if (!forwarding || !forwardTargetIds.length) return; try { for (const targetId of forwardTargetIds) await sendMessage(targetId, forwarding.body, forwarding.media ?? undefined, forwarding.mediaItems); setForwarding(null); setForwardTargetIds([]); } catch { setError("Không thể chuyển tiếp media đến tất cả người nhận đã chọn."); } };
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#F4F7FB]" className="flex-1"><Stack.Screen options={{ headerShown: false }} /><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}><View style={[styles.header, chatLayoutStyles.header]}><IconButton name="chevron.left" onPress={() => router.back()} accessibilityLabel="Quay lại" /><View style={[styles.peerHeader, chatLayoutStyles.peerHeader]}>{avatar ? <Avatar participant={avatar} size={38} /> : null}<View style={styles.peerCopy}><Text numberOfLines={1} style={styles.peerName}>{peer?.displayName ?? "Cuộc trò chuyện"}</Text><Text numberOfLines={1} style={[styles.peerStatus, chatLayoutStyles.peerStatus, (online || peerTyping) && styles.online]}>{peerTyping ? "Đang gõ…" : online ? "Đang hoạt động" : peer ? `@${peer.username}` : ""}</Text></View></View><View style={[styles.callActions, chatLayoutStyles.callActions]}><IconButton name="phone.fill" onPress={() => peer && router.push({ pathname: "/call", params: { peerId: String(peer.id), peerName: peer.displayName, mode: "audio" } })} accessibilityLabel="Gọi thoại" color="#1577E8" /><IconButton name="video.fill" onPress={() => peer && router.push({ pathname: "/call", params: { peerId: String(peer.id), peerName: peer.displayName, mode: "video" } })} accessibilityLabel="Gọi video" color="#1577E8" /><IconButton name="rectangle.on.rectangle" onPress={() => peer && router.push({ pathname: "/call", params: { peerId: String(peer.id), peerName: peer.displayName, mode: "video", startScreenShare: "1" } })} accessibilityLabel="Chia sẻ màn hình" color="#1577E8" /></View></View>
    ...
    {loading ? <View style={[styles.loading, { backgroundColor: wallpaper.color }]}><ActivityIndicator color="#1577E8" /></View> : <FlatList style={{ backgroundColor: wallpaper.color }} data={messages} keyExtractor={(item) => String(item.id)} contentContainerStyle={styles.messageList} renderItem={({ item }) => <MessageBubble message={item} isMine={item.senderId === user?.id} onDownload={() => void downloadMedia(item)} onRevoke={() => void recallMessage(item)} onForward={() => void beginForward(item)} onReply={() => setReplyingTo(item)} onOpenImage={setFullScreenImage} onOpenActions={() => setActionMessage(item)} />} ListEmptyComponent={<Text style={styles.empty}>Chưa có tin nhắn. Hãy gửi lời chào.</Text>} />}
    {fullScreenImage ? <Modal visible transparent animationType="fade" onRequestClose={() => setFullScreenImage(null)}><View style={imageViewerStyles.overlay}><Pressable style={imageViewerStyles.close} onPress={() => setFullScreenImage(null)} accessibilityLabel="Đóng ảnh phóng to"><Text style={imageViewerStyles.closeText}>Đóng</Text></Pressable><Image source={{ uri: mobileApi.mediaUrl(fullScreenImage.url) }} style={imageViewerStyles.image} resizeMode="contain" accessibilityLabel={fullScreenImage.name} /><Text numberOfLines={1} style={imageViewerStyles.name}>{fullScreenImage.name}</Text></View></Modal> : null}
    {peerTyping ? <View style={styles.typingBar}><Text style={styles.typingText}>{peer?.displayName ?? "Người kia"} đang gõ…</Text><TypingDots /></View> : null}
    ...
    {actionMessage ? <View style={styles.forwardSheet}><View style={styles.historyHeader}><Text style={styles.forwardTitle}>Quản lý tin nhắn</Text><Pressable onPress={() => setActionMessage(null)}><Text style={styles.dismiss}>Đóng</Text></Pressable></View><Text style={styles.forwardHint}>Giữ lâu một tin nhắn để mở các thao tác này.</Text>{actionMessage.senderId === user?.id && Date.now() - new Date(`${actionMessage.createdAt}Z`).getTime() <= 15 * 60 * 1000 ? <Pressable onPress={() => void recallMessage(actionMessage)} style={styles.forwardSubmit}><Text style={styles.forwardSubmitText}>Thu hồi cho mọi người</Text></Pressable> : null}<Pressable onPress={() => void deleteMessage(actionMessage)} style={destructiveStyles.button}><Text style={destructiveStyles.buttonText}>Xóa ở phía tôi</Text></Pressable></View> : null}
    {showClearConfirm ? <View style={styles.forwardSheet}><View style={styles.historyHeader}><Text style={styles.forwardTitle}>Làm mới hội thoại?</Text><Pressable onPress={() => setShowClearConfirm(false)}><Text style={styles.dismiss}>Đóng</Text></Pressable></View><Text style={styles.forwardHint}>Lịch sử cũ sẽ không hiển thị lại trên tài khoản này. Tin nhắn mới sẽ bắt đầu một lịch sử mới.</Text><Pressable onPress={() => void clearConversation()} style={destructiveStyles.button}><Text style={destructiveStyles.buttonText}>Làm mới và xóa lịch sử cũ</Text></Pressable></View> : null}
    ...
    {pendingFile ? <View style={chatAttachmentStyles.pendingFile}><View style={chatAttachmentStyles.fileGlyph}><Text style={chatAttachmentStyles.fileGlyphText}>TỆP</Text></View><View style={chatAttachmentStyles.fileCopy}><Text numberOfLines={1} style={chatAttachmentStyles.fileName}>{pendingFile.name}</Text><Text style={chatAttachmentStyles.fileSize}>{formatFileSize(pendingFile.size ?? 0)}</Text></View><Pressable onPress={() => setPendingFile(null)} disabled={uploading}><Text style={styles.dismiss}>Hủy</Text></Pressable><Pressable onPress={() => void sendPendingFile()} disabled={uploading} style={[chatAttachmentStyles.fileSend, uploading && styles.forwardSubmitDisabled]}><Text style={chatAttachmentStyles.fileSendText}>{uploading ? "Đang tải" : "Gửi"}</Text></Pressable></View> : null}
    ...
    <View style={composerStyles.shell}>
      {replyingTo ? <View style={quickReplyStyles.banner}><View style={quickReplyStyles.bannerCopy}><Text style={quickReplyStyles.bannerLabel}>ĐANG TRẢ LỜI</Text><Text numberOfLines={1} style={quickReplyStyles.bannerText}>{replyingTo.body || "Tệp đính kèm"}</Text></View><Pressable onPress={() => setReplyingTo(null)} accessibilityLabel="Hủy trả lời nhanh"><Text style={quickReplyStyles.close}>×</Text></Pressable></View> : null}
...
function MessageBubble({ message, isMine, onDownload, onRevoke, onForward, onReply, onOpenImage, onOpenActions }: { message: MobileMessage; isMine: boolean; onDownload: () => void; onRevoke: () => void; onForward: () => void; onReply: () => void; onOpenImage: (media: NonNullable<MobileMessage["media"]>) => void; onOpenActions: () => void }) {
  ...
  const replyBoundary = message.body.startsWith("↩ ") ? message.body.indexOf("\n") : -1;
  const quotedReply = replyBoundary > 2 ? message.body.slice(2, replyBoundary).trim() : null;
  const messageBody = quotedReply ? message.body.slice(replyBoundary + 1).trim() : message.body;
  return <View {...panResponder.panHandlers} style={[styles.messageRow, isMine && styles.mineRow]}><View style={[chatLayoutStyles.messageGroup, isMine && chatLayoutStyles.mineMessageGroup]}><Pressable onLongPress={onOpenActions} delayLongPress={360} style={[styles.bubble, chatLayoutStyles.bubble, isMine ? styles.mineBubble : styles.peerBubble]}>{mediaItems.length ? <View style={mediaItems.length > 1 ? mediaStyles.multiMedia : undefined}>{mediaItems.map((media) => <View key={media.url} style={mediaStyles.mediaCell}><MediaAttachment media={media} compact={mediaItems.length > 1} onOpenImage={onOpenImage} />{media.caption ? <Text style={[mediaStyles.mediaCaption, chatLayoutStyles.mediaCaption, isMine && mediaStyles.mineMediaCaption]}>{media.caption}</Text> : null}</View>)}</View> : message.mediaRevokedAt ? <Text style={mediaStyles.revoked}>Tệp đa phương tiện đã được thu hồi</Text> : null}{quotedReply ? <View style={[quickReplyStyles.quoteCard, isMine && quickReplyStyles.mineQuoteCard]}><Text style={[quickReplyStyles.quoteLabel, isMine && quickReplyStyles.mineQuoteLabel]}>TRẢ LỜI</Text><Text numberOfLines={2} style={[quickReplyStyles.quoteText, isMine && quickReplyStyles.mineQuoteText]}>{quotedReply}</Text></View> : null}{messageBody ? <Text style={[styles.messageText, chatLayoutStyles.messageText, isMine && styles.mineText]}>{messageBody}</Text> : null}</Pressable>{mediaItems.length ? <View style={[mediaStyles.mediaActions, isMine && mediaStyles.mineMediaActions]}><Pressable onPress={onDownload}><Text style={mediaStyles.mediaActionText}>Tải xuống</Text></Pressable><Pressable onPress={onForward}><Text style={mediaStyles.mediaActionText}>Chuyển tiếp</Text></Pressable>{canRecall ? <Pressable onPress={onRevoke}><Text style={mediaStyles.recallText}>Thu hồi</Text></Pressable> : null}</View> : null}<View style={[chatLayoutStyles.metadata, isMine && chatLayoutStyles.mineMetadata]}><Text style={[styles.time, chatLayoutStyles.time, isMine && styles.mineTime]}>{label}</Text>{isMine ? <Text style={[styles.readReceipt, chatLayoutStyles.readReceipt]}>{deliveryStatus}</Text> : null}</View></View></View>;
}
function MediaAttachment({ media, compact = false, onOpenImage }: { media: NonNullable<MobileMessage["media"]>; compact?: boolean; onOpenImage: (media: NonNullable<MobileMessage["media"]>) => void }) { const url = mobileApi.mediaUrl(media.url); if (media.kind === "image") return <Pressable onPress={() => onOpenImage(media)} accessibilityLabel={`Xem phóng to ${media.name}`}><Image source={{ uri: url }} accessibilityLabel={media.name} style={compact ? mediaStyles.compactMedia : styles.imageMedia} resizeMode="cover" /></Pressable>; if (media.kind === "video") return <VideoAttachment url={url} compact={compact} />; return <View style={chatAttachmentStyles.fileCard}><View style={chatAttachmentStyles.fileGlyph}><Text style={chatAttachmentStyles.fileGlyphText}>TỆP</Text></View><View style={chatAttachmentStyles.fileCopy}><Text numberOfLines={2} style={chatAttachmentStyles.fileName}>{media.name}</Text><Text style={chatAttachmentStyles.fileSize}>{formatFileSize(media.size)}</Text></View></View>; }
...
function formatCallDuration(seconds: number) { return `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, "0")}`; }
function formatFileSize(bytes: number) { if (!bytes) return "Tệp đính kèm"; return bytes < 1024 * 1024 ? `${Math.max(1, Math.round(bytes / 1024))} KB` : `${(bytes / (1024 * 1024)).toFixed(1)} MB`; }
function TypingDots() { const values = [useRef(new Animated.Value(0.28)).current, useRef(new Animated.Value(0.28)).current, useRef(new Animated.Value(0.28)).current]; useEffect(() => { const animation = Animated.loop(Animated.stagger(115, values.map((value) => Animated.sequence([Animated.timing(value, { toValue: 1, duration: 210, useNativeDriver: true }), Animated.timing(value, { toValue: 0.28, duration: 210, useNativeDriver: true })])))); animation.start(); return () => animation.stop(); }, [values]); return <View style={styles.typingDots}>{values.map((opacity, index) => <Animated.View key={index} style={[styles.dot, { opacity, transform: [{ translateY: opacity.interpolate({ inputRange: [0.28, 1], outputRange: [0, -3] }) }] }]} />)}</View>; }

const imageViewerStyles = StyleSheet.create({ overlay: { flex: 1, alignItems: "center", justifyContent: "center", padding: 20, backgroundColor: "rgba(3,16,31,0.96)" }, close: { alignSelf: "flex-end", paddingHorizontal: 13, paddingVertical: 8, borderRadius: 10, backgroundColor: "rgba(255,255,255,0.16)" }, closeText: { color: "#FFFFFF", fontSize: 13, fontWeight: "900" }, image: { flex: 1, alignSelf: "stretch", marginVertical: 18 }, name: { maxWidth: "90%", color: "#DCEBFA", fontSize: 12 } });

const destructiveStyles = StyleSheet.create({ button: { marginTop: 10, minHeight: 40, alignItems: "center", justifyContent: "center", borderRadius: 12, backgroundColor: "#FFF0F0", borderWidth: 1, borderColor: "#F2B8BD" }, buttonText: { color: "#B53F49", fontSize: 12, fontWeight: "900" } });

const mediaStyles = StyleSheet.create({ uploadTrack: { height: 26, marginHorizontal: 12, marginBottom: 3, borderRadius: 10, overflow: "hidden", justifyContent: "center", backgroundColor: "#E5F0FA" }, uploadFill: { position: "absolute", left: 0, top: 0, bottom: 0, borderRadius: 10, backgroundColor: "#B9DBFB" }, uploadLabel: { alignSelf: "center", color: "#236A9F", fontSize: 10, fontWeight: "900" }, cancelUpload: { position: "absolute", right: 6, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 7, backgroundColor: "#FFFFFF" }, cancelUploadText: { color: "#B74A55", fontSize: 10, fontWeight: "900" }, videoPreview: { width: 220, height: 150, borderRadius: 12, overflow: "hidden", marginBottom: 4, backgroundColor: "#112238", alignItems: "center", justifyContent: "center" }, videoPreviewImage: { width: "100%", height: "100%", opacity: 0.82 }, playBadge: { position: "absolute", width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(8,32,57,0.78)" }, playText: { color: "#FFFFFF", fontSize: 18, marginLeft: 2 }, multiMedia: { width: 220, flexDirection: "row", flexWrap: "wrap", gap: 4, marginBottom: 4 }, mediaCell: { maxWidth: 220, gap: 3 }, compactMedia: { width: 108, height: 92, borderRadius: 10, overflow: "hidden", backgroundColor: "#112238" }, mediaCaption: { maxWidth: 220, color: "#25415D", fontSize: 11, lineHeight: 15 }, mineMediaCaption: { color: "#0A4A87" }, callEvent: { minWidth: 184, paddingHorizontal: 13, paddingVertical: 11, borderRadius: 14, flexDirection: "row", alignItems: "center", gap: 9, backgroundColor: "#EAF4FF" }, callIcon: { color: "#1577E8", fontSize: 20, fontWeight: "900" }, callTitle: { color: "#1C527F", fontSize: 13, fontWeight: "900" }, callDetail: { marginTop: 2, color: "#7390AA", fontSize: 10 }, revoked: { color: "#788A9E", fontSize: 12, fontStyle: "italic" }, mediaActions: { marginTop: 4, flexDirection: "row", gap: 12 }, mineMediaActions: { justifyContent: "flex-end" }, mediaActionText: { color: "#44789D", fontSize: 10, fontWeight: "800" }, recallText: { color: "#C25058", fontSize: 10, fontWeight: "800" } });

const chatLayoutStyles = StyleSheet.create({
  header: { minHeight: 64, height: undefined, paddingTop: 8, paddingBottom: 8, gap: 2 },
  peerHeader: { minWidth: 82, paddingRight: 4 },
  peerStatus: { flexShrink: 1 },
  callActions: { flexShrink: 0 },
  messageGroup: { maxWidth: "75%", alignItems: "flex-start", flexShrink: 1 },
  mineMessageGroup: { alignItems: "flex-end" },
  bubble: { maxWidth: "100%", alignSelf: "flex-start", paddingHorizontal: 13, paddingVertical: 10 },
  messageText: { flexShrink: 1, fontSize: 14, lineHeight: 21, ...(Platform.OS === "web" ? ({ wordBreak: "break-word", overflowWrap: "anywhere" } as any) : {}) },
  mediaCaption: { flexShrink: 1, ...(Platform.OS === "web" ? ({ wordBreak: "break-word", overflowWrap: "anywhere" } as any) : {}) },
  metadata: { minHeight: 16, marginTop: 3, flexDirection: "row", alignItems: "center", gap: 6 },
...
const chatAttachmentStyles = StyleSheet.create({
  pendingFile: { minHeight: 66, marginHorizontal: 12, marginBottom: 7, paddingHorizontal: 11, gap: 9, flexDirection: "row", alignItems: "center", borderRadius: 15, borderWidth: 1, borderColor: "#B9D8F3", backgroundColor: "#FFFFFF" },
  fileCard: { minWidth: 180, maxWidth: 245, minHeight: 62, paddingHorizontal: 10, gap: 9, flexDirection: "row", alignItems: "center", borderRadius: 12, backgroundColor: "#F4F9FE", borderWidth: 1, borderColor: "#CFE4F5" },
  fileGlyph: { width: 38, height: 40, alignItems: "center", justifyContent: "center", borderRadius: 10, backgroundColor: "#DDEEFF" },
  fileGlyphText: { color: "#2275B8", fontSize: 9, fontWeight: "900" },
  fileCopy: { flex: 1, minWidth: 0 },
  fileName: { color: "#234C71", fontSize: 12, lineHeight: 17, fontWeight: "800" },
  fileSize: { marginTop: 2, color: "#7891A8", fontSize: 10 },
  fileSend: { minHeight: 31, paddingHorizontal: 10, alignItems: "center", justifyContent: "center", borderRadius: 9, backgroundColor: "#1577E8" },
  fileSendText: { color: "#FFFFFF", fontSize: 11, fontWeight: "900" },
  fileTrigger: { width: 34, height: 44, alignItems: "center", justifyContent: "center", borderRadius: 13, borderWidth: 1, borderColor: "#E0EAF3", backgroundColor: "#F7FAFD" },
  fileTriggerText: { color: "#287BC1", fontSize: 23, fontWeight: "800" },
});

const quickReplyStyles = StyleSheet.create({
  banner: { minHeight: 45, marginBottom: 8, paddingHorizontal: 10, gap: 10, flexDirection: "row", alignItems: "center", borderLeftWidth: 3, borderLeftColor: "#1577E8", borderRadius: 10, backgroundColor: "#EFF7FF" },
  bannerCopy: { flex: 1, minWidth: 0 },
  bannerLabel: { color: "#4380B5", fontSize: 8, fontWeight: "900", letterSpacing: 0.7 },
  bannerText: { marginTop: 1, color: "#23527C", fontSize: 11, fontWeight: "700" },
  close: { width: 28, height: 28, textAlign: "center", textAlignVertical: "center", borderRadius: 14, color: "#3978AE", fontSize: 21, lineHeight: 27, backgroundColor: "#DCEEFF" },
  quoteCard: { marginBottom: 7, paddingHorizontal: 9, paddingVertical: 7, borderLeftWidth: 3, borderLeftColor: "#6CADE5", borderRadius: 8, backgroundColor: "#EEF6FD" },
  mineQuoteCard: { borderLeftColor: "#3782BF", backgroundColor: "#CFE6FA" },
  quoteLabel: { color: "#5E89AE", fontSize: 8, fontWeight: "900", letterSpacing: 0.65 },
  mineQuoteLabel: { color: "#2C6796" },
  quoteText: { marginTop: 2, color: "#55748D", fontSize: 11, lineHeight: 15, fontWeight: "600" },
  mineQuoteText: { color: "#245B86" },
});
...
const composerStyles = StyleSheet.create({
  ...
  previewText: { color: "#1A4D79", fontSize: 12, lineHeight: 17, fontWeight: "600", marginTop: 1 },
  previewPlaceholder: { color: "#92A2B2", fontWeight: "500" },
  actionRow: { minHeight: 48, flexDirection: "row", alignItems: "center", gap: 8, marginTop: 8 },
  attach: { width: 42, height: 44, alignItems: "center", justifyContent: "center", borderRadius: 14, borderWidth: 1, borderColor: "#CFE4F8", backgroundColor: "#EAF5FF" },
  attachDisabled: { opacity: 0.48 },
  attachText: { color: "#1577E8", fontSize: 25, fontWeight: "400", lineHeight: 28 },
  input: { flex: 1, minHeight: 44, maxHeight: 88, paddingHorizontal: 13, paddingVertical: 9, borderRadius: 14, borderWidth: 1, borderColor: "#E0EAF3", backgroundColor: "#FFFFFF", color: "#153150", fontSize: 14, lineHeight: 19 },
  send: { width: 46, height: 44, alignItems: "center", justifyContent: "center", borderRadius: 15, backgroundColor: "#1577E8", shadowColor: "#0E64C4", shadowOpacity: 0.18, shadowRadius: 7, shadowOffset: { width: 0, height: 3 }, elevation: 3 },
  sendDisabled: { backgroundColor: "#A9C7E4", shadowOpacity: 0 },
});

const styles = StyleSheet.create({ flex: { flex: 1 }, header: { height: 64, paddingHorizontal: 10, flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", borderBottomWidth: 1, borderBottomColor: "#E4ECF4", shadowColor: "#153150", shadowOpacity: 0.03, shadowRadius: 7, shadowOffset: { width: 0, height: 3 }, elevation: 1 }, peerHeader: { flex: 1, minWidth: 0, flexDirection: "row", alignItems: "center", gap: 9 }, peerCopy: { minWidth: 0, flex: 1 }, peerName: { color: "#153150", fontWeight: "900", fontSize: 15 }, peerStatus: { marginTop: 1, color: "#8392A4", fontSize: 10 }, online: { color: "#198A68", fontWeight: "700" }, callActions: { flexDirection: "row" }, loading: { flex: 1, alignItems: "center", justifyContent: "center" }, messageList: { flexGrow: 1, paddingHorizontal: 15, paddingTop: 16, paddingBottom: 24, gap: 8 }, messageRow: { flexDirection: "row", justifyContent: "flex-start" }, mineRow: { justifyContent: "flex-end" }, bubble: { maxWidth: "80%", borderRadius: 18, paddingHorizontal: 12, paddingVertical: 9 }, peerBubble: { backgroundColor: "#FFFFFF", borderTopLeftRadius: 5 }, mineBubble: { backgroundColor: "#D9ECFF", borderTopRightRadius: 5 }, messageText: { color: "#25415D", fontSize: 14, lineHeight: 20 }, mineText: { color: "#0A4A87" }, imageMedia: { width: 220, height: 180, borderRadius: 12, marginBottom: 4, backgroundColor: "#DCE8F4" }, videoMedia: { width: 220, height: 150, borderRadius: 12, overflow: "hidden", marginBottom: 4, backgroundColor: "#112238" }, time: { textAlign: "right", marginTop: 3, color: "#9AA8B8", fontSize: 9 }, mineTime: { color: "#6F98C0" }, readReceipt: { marginTop: 3, color: "#6F98C0", fontSize: 10, fontWeight: "700", textAlign: "right" }, empty: { marginTop: 38, color: "#8A98AA", fontSize: 13, textAlign: "center" }, typingBar: { marginHorizontal: 12, marginBottom: 6, paddingHorizontal: 11, paddingVertical: 7, alignSelf: "flex-start", flexDirection: "row", alignItems: "center", gap: 7, borderRadius: 12, backgroundColor: "#EAF4FF" }, typingText: { color: "#3672A7", fontSize: 11, fontWeight: "700" }, typingDots: { flexDirection: "row", alignItems: "center", gap: 3 }, dot: { width: 4, height: 4, borderRadius: 2, backgroundColor: "#4A91CE" }, dotMiddle: { opacity: 0.65 }, error: { marginHorizontal: 12, marginBottom: 7, padding: 9, borderRadius: 11, flexDirection: "row", alignItems: "center", backgroundColor: "#FFF0F0" }, errorText: { flex: 1, color: "#BF3840", fontSize: 11 }, dismiss: { color: "#1577E8", fontSize: 11, fontWeight: "800" }, historySheet: { marginHorizontal: 12, marginBottom: 7, padding: 13, borderRadius: 15, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#DCE8F4" }, historyHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 }, historyTitle: { flex: 1, color: "#173451", fontSize: 13, fontWeight: "900" }, historyItem: { paddingTop: 9, paddingBottom: 2, borderTopWidth: 1, borderTopColor: "#EDF3F8", marginTop: 8 }, historyItemTitle: { color: "#315B82", fontSize: 12, fontWeight: "800" }, historyItemDetail: { color: "#7890A5", fontSize: 10, marginTop: 2 }, historyEmpty: { color: "#7890A5", fontSize: 11, marginTop: 10 }, forwardSheet: { marginHorizontal: 12, marginBottom: 7, padding: 13, borderRadius: 15, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#B9D8F3" }, forwardTitle: { flex: 1, color: "#173451", fontSize: 13, fontWeight: "900" }, forwardHint: { color: "#7890A5", fontSize: 11, marginTop: 4, marginBottom: 8 }, forwardTarget: { minHeight: 43, flexDirection: "row", alignItems: "center", gap: 9, borderTopWidth: 1, borderTopColor: "#EDF3F8" }, forwardTargetSelected: { backgroundColor: "#F0F8FF" }, forwardCheck: { width: 19, height: 19, alignItems: "center", justifyContent: "center", borderRadius: 6, borderWidth: 1, borderColor: "#9AB6D0" }, forwardCheckSelected: { backgroundColor: "#1577E8", borderColor: "#1577E8" }, forwardCheckMark: { color: "#FFFFFF", fontSize: 13, fontWeight: "900" }, forwardTargetText: { color: "#315B82", fontSize: 12, fontWeight: "700" }, forwardSubmit: { marginTop: 10, minHeight: 40, alignItems: "center", justifyContent: "center", borderRadius: 12, backgroundColor: "#1577E8" }, forwardSubmitDisabled: { opacity: 0.48 }, forwardSubmitText: { color: "#FFFFFF", fontSize: 12, fontWeight: "900" }, mediaComposer: { maxHeight: 290, marginHorizontal: 12, marginBottom: 7, padding: 12, borderRadius: 15, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#B9D8F3" }, mediaComposerTitle: { color: "#173451", fontSize: 13, fontWeight: "900" }, mediaComposerHint: { maxWidth: 260, color: "#7890A5", fontSize: 10, marginTop: 2 }, pendingMediaRow: { flexDirection: "row", alignItems: "center", gap: 8, borderTopWidth: 1, borderTopColor: "#EDF3F8", paddingTop: 8, marginTop: 7 }, pendingThumbnail: { width: 44, height: 44, borderRadius: 9, backgroundColor: "#DCE8F4" }, pendingVideo: { width: 44, height: 44, alignItems: "center", justifyContent: "center", borderRadius: 9, backgroundColor: "#173451" }, pendingVideoText: { color: "#DDEEFF", fontSize: 8, fontWeight: "900" }, pendingCaption: { flex: 1, minHeight: 42, maxHeight: 62, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 10, backgroundColor: "#F1F5F9", color: "#153150", fontSize: 12 }, mediaSubmit: { minHeight: 41, marginTop: 10, alignItems: "center", justifyContent: "center", borderRadius: 12, backgroundColor: "#1577E8" }, mediaSubmitText: { color: "#FFFFFF", fontSize: 12, fontWeight: "900" }, composer: { minHeight: 68, paddingHorizontal: 12, paddingTop: 9, paddingBottom: 12, gap: 8, flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", borderTopWidth: 1, borderTopColor: "#E4ECF4", shadowColor: "#153150", shadowOpacity: 0.06, shadowRadius: 9, shadowOffset: { width: 0, height: -3 }, elevation: 4 }, attach: { width: 36, height: 42, alignItems: "center", justifyContent: "center", borderRadius: 13, backgroundColor: "#EAF4FF" }, attachDisabled: { opacity: 0.5 }, attachText: { color: "#1577E8", fontSize: 24, fontWeight: "400", lineHeight: 28 }, input: { flex: 1, minHeight: 42, maxHeight: 96, paddingHorizontal: 13, paddingVertical: 8, borderRadius: 14, backgroundColor: "#F1F5F9", color: "#153150", fontSize: 14 }, send: { width: 44, height: 44, alignItems: "center", justifyContent: "center", borderRadius: 15, backgroundColor: "#1577E8" }, sendDisabled: { backgroundColor: "#A4C8ED" }, pressed: { opacity: 0.65 } });
