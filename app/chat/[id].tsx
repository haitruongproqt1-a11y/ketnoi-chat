...
export default function ChatScreen() {
  ...
  const pickMedia = async () => {
    ...
    try {
      ...
      const optimized = await optimizeMedia({ uri: asset.uri, name: asset.fileName ?? `${kind}-${Date.now()}.${kind === "video" ? "mp4" : "jpg"}`, mimeType, size, kind, width: asset.width, height: asset.height }, (progress) => setUploadProgress(Math.min(0.55, progress * 0.55)), (cancel) => { uploadCancellation.current = cancel; });
      if (uploadWasCanceled.current) throw new Error("UPLOAD_CANCELLED");
      if (optimized.size > 12 * 1024 * 1024) { setError("Tệp sau khi tối ưu vẫn vượt quá giới hạn 12 MB."); return; }
      ...
      if (kind === "video" && Platform.OS !== "web") {
        try {
          ...
          const thumbnailMedia = await mobileApi.uploadMedia(token, { uri: thumbnail.uri, name: `thumbnail-${Date.now()}.jpg`, mimeType: "image/jpeg", size: 1 }, (progress) => setUploadProgress(0.55 + progress * 0.1), (cancel) => { uploadCancellation.current = cancel; });
          thumbnailUrl = thumbnailMedia.url;
        } catch { thumbnailUrl = null; }
      }
      const media = await mobileApi.uploadMedia(token, optimized, (progress) => setUploadProgress(0.65 + progress * 0.35), (cancel) => { uploadCancellation.current = cancel; });
