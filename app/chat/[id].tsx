...
export default function ChatScreen() {
  ...
  const pickMedia = async () => {
    ...
    try {
      ...
      const media = await mobileApi.uploadMedia(token, { uri: asset.uri, name: asset.fileName ?? `${kind}-${Date.now()}.${kind === "video" ? "mp4" : "jpg"}`, mimeType, size });
      const message = await sendMessage(peer.id, "", media);
      setMessages((current) => current.some((item) => item.id === message.id) ? current : [...current, message]);
...
function VideoAttachment({ url }: { url: string }) { const player = useVideoPlayer({ uri: url, useCaching: true }); return <VideoView player={player} style={styles.videoMedia} nativeControls allowsFullscreen contentFit="cover" surfaceType="textureView" />; }
