...
export default function ChatScreen() {
  ...
  const pickMedia = async () => {
    ...
    try {
      const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.All, quality: 0.8, videoMaxDuration: 90 });
      if (result.canceled || !result.assets[0]) return;
      const asset = result.assets[0];
      const size = asset.fileSize ?? 0;
      if (size > 12 * 1024 * 1024) { setError("Ảnh hoặc video cần nhỏ hơn 12 MB."); return; }
      const mimeType = asset.mimeType ?? (asset.type === "video" ? "video/mp4" : "image/jpeg");
      const kind = mimeType.startsWith("video/") ? "video" : "image";
      if (!["image", "video"].includes(kind)) { setError("Chỉ hỗ trợ ảnh và video."); return; }
      setUploading(true); setUploadProgress(0); setError("");
      let thumbnailUrl: string | null = null;
      if (kind === "video" && Platform.OS !== "web") {
        try {
          const thumbnail = await VideoThumbnails.getThumbnailAsync(asset.uri, { time: 800, quality: 0.7 });
          const thumbnailMedia = await mobileApi.uploadMedia(token, { uri: thumbnail.uri, name: `thumbnail-${Date.now()}.jpg`, mimeType: "image/jpeg", size: 1 });
          thumbnailUrl = thumbnailMedia.url;
        } catch { thumbnailUrl = null; }
      }
      const media = await mobileApi.uploadMedia(token, { uri: asset.uri, name: asset.fileName ?? `${kind}-${Date.now()}.${kind === "video" ? "mp4" : "jpg"}`, mimeType, size }, setUploadProgress);
      const message = await sendMessage(peer.id, "", { ...media, thumbnailUrl });
      setMessages((current) => current.some((item) => item.id === message.id) ? current : [...current, message]);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Không thể gửi ảnh hoặc video."); }
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#F4F7FB]" className="flex-1"><Stack.Screen options={{ headerShown: false }} /><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}><View style={styles.header}><IconButton name="chevron.left" onPress={() => router.back()} accessibilityLabel="Quay lại" /><View style={styles.peerHeader}>{avatar ? <Avatar participant={avatar} size={38} /> : null}<View style={styles.peerCopy}><Text numberOfLines={1} style={styles.peerName}>{peer?.displayName ?? "Cuộc trò chuyện"}</Text><Text style={[styles.peerStatus, (online || peerTyping) && styles.online]}>{peerTyping ? "Đang gõ…" : online ? "Đang hoạt động" : peer ? `@${peer.username}` : ""}</Text></View></View><View style={styles.callActions}><IconButton name="phone.fill" onPress={() => peer && router.push({ pathname: "/call", params: { peerId: String(peer.id), peerName: peer.displayName, mode: "audio" } })} accessibilityLabel="Gọi thoại" color="#1577E8" /><IconButton name="video.fill" onPress={() => peer && router.push({ pathname: "/call", params: { peerId: String(peer.id), peerName: peer.displayName, mode: "video" } })} accessibilityLabel="Gọi video" color="#1577E8" /></View></View>
    ...
    {uploading ? <View style={mediaStyles.uploadTrack}><View style={[mediaStyles.uploadFill, { width: `${Math.max(5, Math.round(uploadProgress * 100))}%` }]} /><Text style={mediaStyles.uploadLabel}>Đang tải {Math.round(uploadProgress * 100)}%</Text></View> : null}
...
function MessageBubble({ message, isMine, onDownload, onRevoke }: { message: MobileMessage; isMine: boolean; onDownload: () => void; onRevoke: () => void }) {
  ...
  return <View style={[styles.messageRow, isMine && styles.mineRow]}><View><View style={[styles.bubble, isMine ? styles.mineBubble : styles.peerBubble]}>{message.media ? <MediaAttachment media={message.media} /> : message.mediaRevokedAt ? <Text style={mediaStyles.revoked}>Tệp đa phương tiện đã được thu hồi</Text> : null}{message.body ? <Text style={[styles.messageText, isMine && styles.mineText]}>{message.body}</Text> : null}<Text style={[styles.time, isMine && styles.mineTime]}>{label}</Text></View>{message.media ? <View style={[mediaStyles.mediaActions, isMine && mediaStyles.mineMediaActions]}><Pressable onPress={onDownload}><Text style={mediaStyles.mediaActionText}>Tải xuống</Text></Pressable>{isMine ? <Pressable onPress={onRevoke}><Text style={mediaStyles.recallText}>Thu hồi</Text></Pressable> : null}</View> : null}{isMine ? <Text style={styles.readReceipt}>{deliveryStatus}</Text> : null}</View></View>;
}
function MediaAttachment({ media }: { media: NonNullable<MobileMessage["media"]> }) { const url = mobileApi.mediaUrl(media.url); if (media.kind === "image") return <Image source={{ uri: url }} accessibilityLabel={media.name} style={styles.imageMedia} resizeMode="cover" />; return <VideoAttachment url={url} />; }
function VideoAttachment({ url }: { url: string }) { const [play, setPlay] = useState(false); const player = useVideoPlayer(play ? { uri: url, useCaching: true } : null); return play ? <VideoView player={player} style={styles.videoMedia} nativeControls allowsFullscreen contentFit="cover" surfaceType="textureView" /> : <Pressable onPress={() => setPlay(true)} style={mediaStyles.videoPreview}><Image source={{ uri: url }} style={mediaStyles.videoPreviewImage} /><View style={mediaStyles.playBadge}><Text style={mediaStyles.playText}>▶</Text></View></Pressable>; }

const mediaStyles = StyleSheet.create({ uploadTrack: { height: 26, marginHorizontal: 12, marginBottom: 3, borderRadius: 10, overflow: "hidden", justifyContent: "center", backgroundColor: "#E5F0FA" }, uploadFill: { position: "absolute", left: 0, top: 0, bottom: 0, borderRadius: 10, backgroundColor: "#B9DBFB" }, uploadLabel: { alignSelf: "center", color: "#236A9F", fontSize: 10, fontWeight: "900" }, videoPreview: { width: 220, height: 150, borderRadius: 12, overflow: "hidden", marginBottom: 4, backgroundColor: "#112238", alignItems: "center", justifyContent: "center" }, videoPreviewImage: { width: "100%", height: "100%", opacity: 0.82 }, playBadge: { position: "absolute", width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(8,32,57,0.78)" }, playText: { color: "#FFFFFF", fontSize: 18, marginLeft: 2 }, revoked: { color: "#788A9E", fontSize: 12, fontStyle: "italic" }, mediaActions: { marginTop: 4, flexDirection: "row", gap: 12 }, mineMediaActions: { justifyContent: "flex-end" }, mediaActionText: { color: "#44789D", fontSize: 10, fontWeight: "800" }, recallText: { color: "#C25058", fontSize: 10, fontWeight: "800" } });
