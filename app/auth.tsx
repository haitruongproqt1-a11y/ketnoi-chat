...
export default function AuthScreen() {
  ...
  const selectedQuestion = SECRET_QUESTIONS.find((question) => question.id === secretQuestion) ?? SECRET_QUESTIONS[0];
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={styles.page}><View style={styles.hero}><View style={styles.mark}><View style={styles.bubble} /><View style={styles.spark} /></View><Text style={styles.title}>Kết Nối</Text><Text style={styles.subtitle}>Nhắn tin và gọi trực tiếp, gần gũi mỗi ngày.</Text></View>
          <View style={styles.card}>
            ...
            {mode === "register" && registerStep === 2 ? <><Text style={styles.stepTitle}>Thiết lập khôi phục tài khoản</Text><Text style={styles.stepHint}>Chọn câu hỏi phù hợp và nhập câu trả lời chỉ bạn biết.</Text><Text style={styles.label}>Câu hỏi bí mật</Text><View style={questionStyles.pickerWrap}><Pressable accessibilityRole="button" accessibilityLabel="Chọn câu hỏi bí mật" disabled={loading} onPress={() => setQuestionMenuOpen((open) => !open)} style={({ pressed }) => [questionStyles.picker, questionMenuOpen && questionStyles.pickerOpen, pressed && styles.pressed]}><Text numberOfLines={1} style={questionStyles.pickerText}>{selectedQuestion.label}</Text><Text style={questionStyles.pickerArrow}>{questionMenuOpen ? "⌃" : "⌄"}</Text></Pressable>{questionMenuOpen ? <View style={questionStyles.menu}>{SECRET_QUESTIONS.map((question) => <Pressable key={question.id} onPress={() => { setSecretQuestion(question.id); setQuestionMenuOpen(false); }} style={({ pressed }) => [questionStyles.option, question.id === secretQuestion && questionStyles.optionSelected, pressed && styles.pressed]}><Text style={[questionStyles.optionText, question.id === secretQuestion && questionStyles.optionTextSelected]}>{question.label}</Text></Pressable>)}</View> : null}</View><Field label="Câu trả lời bí mật" value={secretAnswer} onChangeText={setSecretAnswer} placeholder="Nhập câu trả lời của bạn" autoCapitalize="none" autoCorrect={false} editable={!loading} /><View style={questionStyles.note}><Text style={questionStyles.noteTitle}>Lưu ý quan trọng</Text><Text style={questionStyles.noteText}>Câu hỏi và câu trả lời này dùng để lấy lại mật khẩu. Hãy chọn thông tin bạn dễ nhớ nhưng người khác khó đoán.</Text></View><Pressable onPress={() => { setRegisterStep(1); setQuestionMenuOpen(false); setError(""); }} disabled={loading} style={({ pressed }) => [styles.backRegister, pressed && styles.pressed]}><Text style={styles.backRegisterText}>Sửa thông tin đăng ký</Text></Pressable></> : null}
...
const questionStyles = StyleSheet.create({
  ...
  picker: { minHeight: 48, paddingHorizontal: 13, borderRadius: 13, flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#F4F7FB", borderWidth: 1, borderColor: "#E3EBF4" },
  pickerOpen: { borderColor: "#8DC3FA", backgroundColor: "#EAF5FF" },
  pickerText: { flex: 1, color: "#163252", fontSize: 13, fontWeight: "700" },
  pickerArrow: { color: "#1577E8", fontSize: 20, lineHeight: 21, fontWeight: "900" },
  menu: { marginTop: 6, borderRadius: 13, overflow: "hidden", borderWidth: 1, borderColor: "#DCE8F4", backgroundColor: "#FFFFFF" },
  option: { minHeight: 41, justifyContent: "center", paddingHorizontal: 13, borderBottomWidth: 1, borderBottomColor: "#EDF2F7" },
  optionSelected: { backgroundColor: "#EAF5FF" },
  optionText: { color: "#536B83", fontSize: 12, lineHeight: 17 },
  optionTextSelected: { color: "#1577E8", fontWeight: "800" },
  note: { marginTop: -2, marginBottom: 7, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 12, backgroundColor: "#FFF8E9", borderWidth: 1, borderColor: "#F8E4B8" },
  noteTitle: { color: "#946B1C", fontSize: 11, fontWeight: "900" },
  noteText: { color: "#806C47", marginTop: 3, fontSize: 10, lineHeight: 15 },
});
