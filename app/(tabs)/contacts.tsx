...
export default function ContactsScreen() {
  ...
  const { onlineIds, refreshPendingFriendRequestCount } = useMobileSocket();
  ...
  }, [refreshPendingFriendRequestCount, token]);

  useEffect(() => { void load(); }, [load]);
  useEffect(() => {
    if (!token || !query.trim()) { setResults([]); setSearching(false); return; }
    setSearching(true);
    const handle = setTimeout(() => {
      void mobileApi.searchUsers(token, query).then(setResults).catch((error: Error) => setNotice(error.message)).finally(() => setSearching(false));
    }, 260);
